...
function page(lang, c) {
  ...
  const decisions = c.decisions.map(([heading, p1, p2], i) => {
    ...
    const classes = image ? `issue-card issue-card--media${i === 3 ? ' issue-card--reverse' : ''}` : 'issue-card issue-card--text';
    return `<article class="${classes}">${media}<div class="issue-copy"><span class="issue-number">${String(i + 1).padStart(2, '0')} · ${c.before}</span><h3>${heading}</h3><p>${p1}</p><p>${p2}</p></div></article>`;
  }).join('');
