...
function headerHtml(lang, context, html){
  ...
  return `<header class="site-header shared-site-header"><div class="scroll-progress" aria-hidden="true"><span></span></div><div class="nav-shell"><a class="brand" href="${rootUrl}" aria-label="EcoViva Mallorca"><img src="/assets/logos/ecoviva-horizontal-header.png" alt="EcoViva Mallorca"></a><input class="menu-state" id="mobile-menu-${lang}" type="checkbox" aria-hidden="true"><nav class="main-nav" aria-label="${c.nav}"><div class="nav-dropdown"><button class="nav-dropdown-toggle" type="button" aria-expanded="false">${c.what} <span aria-hidden="true">⌄</span></button><div class="nav-dropdown-menu"><a href="${rootUrl}#what-we-do" data-section-link="what-we-do">${c.all}</a><a href="${roofUrls[lang]}">${c.roof}</a><a href="${facadeUrls[lang]}">${c.facade}</a><a href="${checkUrls[lang]}">${c.check}</a><a href="${rootUrl}#renovation-examples">${c.examples}</a></div></div><a href="${rootUrl}#how-we-work" data-section-link="how-we-work">${c.how}</a><a href="${rootUrl}#why-ecoviva" data-section-link="why-ecoviva">${c.why}</a><a href="${technicalUrl}"${technicalActive?' aria-current="page"':''}>${c.technical}</a><a ${guidesAttrs}>${c.guides}</a><a href="${rootUrl}#areas" data-section-link="areas">${c.areas}</a><a href="${rootUrl}#professionals" data-section-link="professionals">${c.work}</a><div class="nav-dropdown nav-dropdown-right"><button class="nav-dropdown-toggle" type="button" aria-expanded="false">${c.who} <span aria-hidden="true">⌄</span></button><div class="nav-dropdown-menu nav-dropdown-menu-right"><a href="${rootUrl}#about">Markus &amp; Maritza</a><a href="${rootUrl}#areas">${c.office}</a><a href="${brochureUrl}">${c.brochure}</a><a href="${rootUrl}#contact">${c.contact}</a></div></div><a class="button" href="${rootUrl}#renovation-request">${c.cta}</a></nav><nav class="language-nav" aria-label="${c.language}">${['en','es','de'].map(l=>`<a href="${langUrls[l]}" lang="${l}"${l===lang?' aria-current="page"':''}>${l.toUpperCase()}</a>`).join('')}</nav><label class="menu-toggle" for="mobile-menu-${lang}" role="button" aria-label="${c.menu}" aria-expanded="false"><span></span><span></span><span></span></label></div></header>`;
}
